...
echo "轻量级服务器进程已启动 (PID: {$status['pid']})\n";
...
echo "测试 memo.help_prompt 工具...\n";
$requestData = json_encode($helpToolRequest) . "\n";
if (fwrite($pipes[0], $requestData) === false) {
    echo "写入失败: " . error_get_last()['message'] . "\n";
} else {
    echo "请求已发送: " . $requestData;
...
if ($response === false) {
    ...
    echo "memo.help_prompt 响应: " . $response;
}
